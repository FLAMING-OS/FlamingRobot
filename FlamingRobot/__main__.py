...
import FlamingRobot.modules.sql.users_sql as sql
from FlamingRobot import (
    BOT_NAME,
    BOT_USERNAME,
    LOGGER,
    OWNER_ID,
...
from FlamingRobot.modules import ALL_MODULES
from FlamingRobot.modules.helper_funcs.chat_status import is_user_admin
from FlamingRobot.modules.helper_funcs.misc import paginate_modules
...
*ʜᴇʏ* {} , 🥀
*๏ ᴛʜɪs ɪs* {} !
➻ ᴛʜᴇ ᴍᴏsᴛ ᴩᴏᴡᴇʀғᴜʟ ᴛᴇʟᴇɢʀᴀᴍ ᴍᴜsɪᴄ ʙᴏᴛ ᴀɴᴅ ɪ ʜᴀᴠᴇ sᴏᴍᴇ ᴀᴇsᴏᴍᴇ ᴀɴᴅ ᴜsᴇғᴜʟ ғᴇᴀᴛᴜʀᴇs.
...
buttons = [
    ...
    [
        InlineKeyboardButton(text="🏡 ᴀʙᴏᴜᴛ 🏡 ", callback_data="FLAMING_"),
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("FlamingRobot.modules." + module_name)
...
@run_async
def Flaming_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "flaming_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="🚩sᴜᴩᴩᴏʀᴛ🚩", callback_data="flaming_support"
                        ),
                        InlineKeyboardButton(
                    ...
                    [
                        InlineKeyboardButton(text="◁", callback_data="flaming_back"),
                    ],
                ]
            ),
        )
    elif query.data == "flaming_support":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="🚩sᴜᴩᴩᴏʀᴛ🚩", url=f"https://t.me/friendsforever431"
                        ),
                        InlineKeyboardButton(
                            text="💗ᴜᴩᴅᴀᴛᴇs💓", url=f"https://t.me/friendsforever432"
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                    ...
                    [
                        InlineKeyboardButton(text="◁", callback_data="flaming_"),
                    ],
                ]
            ),
        )
    elif query.data == "flaming_back":
        first_name = update.effective_user.first_name 
        query.message.edit_text(
            PM_START_TEXT.format(escape_markdown(first_name), BOT_NAME),
            reply_markup=InlineKeyboardMarkup(buttons),
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 5910057231 and DONATION_LINK:
            update.effective_message.reply_text(
                f"» ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴩᴇʀ ᴏғ {dispatcher.bot.first_name} sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ɪs [ɢɪᴛʜᴜʙ](https://github.com/noob-flaming/FlamingRobot)"
...
─╼⃝𖠁-----------------𖠁⃝╾─•
ᴍᴀᴅᴇ ᴡɪᴛʜ ❤  ʙʏ [PANKAJ]("t.me/pan_kaj_0027")""",
                parse_mode=ParseMode.MARKDOWN,
            )
        except Unauthorized:
            LOGGER.warning(
    ...
    about_callback_handler = CallbackQueryHandler(
        Flaming_about_callback, pattern=r"flaming_"
    )
