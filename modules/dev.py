...
import FlamingRobot
from FlamingRobot import dispatcher
from FlamingRobot.modules.helper_funcs.chat_status import dev_plus
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        update.effective_message.reply_text(f"Current state: {FlamingRobot.ALLOW_CHATS}")
        return
    if args[0].lower() in ["off", "no"]:
        FlamingRobot.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        FlamingRobot.ALLOW_CHATS = False
    else:
